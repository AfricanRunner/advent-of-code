...
class CaveWall:
    ...
    def __init__(self, rock_patterns: list[list[Vec]]):
        max_x = max([max([pos.x for pos in pattern])
                     for pattern in rock_patterns]) + 1
        min_x = min([min([pos.x for pos in pattern])
                     for pattern in rock_patterns]) - 1
        max_y = max([max([pos.y for pos in pattern])
                     for pattern in rock_patterns])
        self._wall = [[Material.AIR for _ in range(max_x - min_x + 1)]
