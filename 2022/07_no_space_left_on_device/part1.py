...
class Directory:
    ...
    def total_sizes_less_than(self, n: int) -> int:
        total = self._size if self._size <= n else 0
        return total + sum([
            sub_directory.total_sizes_less_than(n)
            for sub_directory in self._sub_directories.values()
        ])
...
def main(input: list[str]):
    ...
    root.calculate_size()
    print(root.total_sizes_less_than(100_000))
